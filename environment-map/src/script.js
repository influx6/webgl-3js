...
gui.add(global, 'backgroundBlur').min(0.0).max(1).step(0.1).onChange(updateAllMaterials).onChange(()=>{
    scene.backgroundBlurriness = global.backgroundBlur
})
gui.add(global, 'backgroundIntensity').min(1).max(15).step(1.5).onChange(updateAllMaterials).onChange(()=>{
    scene.backgroundIntensity = global.backgroundIntensity
})
